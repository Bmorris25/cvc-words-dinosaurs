...
function QuestionCard() {
  ...
  const [hatchedChoice, setHatchedChoice] = useState(null);
  ...
  function handleClickChoice(choice) {
    if (hatchedChoice !== null) return;

    if (choice === currentQuestion.answer) {
      setHatchedChoice(choice);
      setFeedback('✅ Correct!');
      setTimeout(() => {
        nextQuestion();
      }, 1000);
  ...
  if (!gameStarted) {
    return (
      <div className="question-card">
        <img
          src={withBase('assets/dino-start.png')}
          alt="start game"
      ...
      <div className="choices">
        {choices.map((choice, index) => (
          <button
            ...
            className="choice-button egg-button"
            disabled={hatchedChoice !== null}
          >
            <img
              src={
                hatchedChoice === choice
                  ? withBase('assets/dino-egg-hatched.png')
                  : withBase('assets/dino-egg.png')
              }
              alt=""
              className="egg-image"
            />
            <span className="egg-letter">{choice}</span>
          </button>
        ))}
      </div>
    </div>
